...
const ResultsHandler = {
    ...
    <div class="bg-white border rounded-lg p-6 hover:shadow-md transition-all">
      ...
      <div class="border-t pt-4">
        ...
        <div class="flex space-x-2 mt-4">
          ${result.buyer?.phone ? `
            <button data-url="https://wa.me/${result.buyer.phone?.replace(/\D/g, '')}"
                    class="contact-button flex-1 bg-green-500 text-white text-center py-2 rounded-lg hover:bg-green-600">
              <i class="fab fa-whatsapp mr-2"></i>WhatsApp
            </button>
            <button data-url="tel:${result.buyer.phone}"
                    class="contact-button flex-1 bg-primary-600 text-white text-center py-2 rounded-lg hover:bg-primary-700">
  ...
  <div class="bg-white border rounded-lg p-6 hover:shadow-md transition-all">
    ...
    <div class="border-t pt-4">
      ...
      <div class="flex space-x-2 mt-4">
        ${result.seller?.phone ? `
          <button data-url="tel:${result.seller.phone}"
                  class="contact-button flex-1 bg-primary-600 text-white text-center py-2 rounded-lg hover:bg-primary-700">
            <i class="fas fa-phone mr-2"></i>Telepon
          </button>
        ` : ''}
...
`;
  ...
  _addEventListeners() {
    ...
    const message = 'Untuk keamanan Anda, harap berhati-hati terhadap penipuan saat bertransaksi di luar platform. Jangan mentransfer uang sebelum memastikan keaslian penjual dan produk.';

    contactButtons.forEach((button) => {
      button.addEventListener('click', (event) => {
        event.preventDefault();
        const url = event.currentTarget.dataset.url;
        AlertHelper.showConfirmation(message, () => {
